...
async function addCleaningSubCategories() {
  try {
    await connectDB();
    console.log('🔄 بدء إضافة أقسام المنظفات والمنتجات...\n');

    // البحث عن الفئة الرئيسية
    const category = await Category.findOne({
      where: { name: cleaningData.categoryName }
    });

    if (!category) {
      console.error(`❌ لم يتم العثور على فئة "${cleaningData.categoryName}"`);
      process.exit(1);
    }
    ...
    // إضافة الأقسام الفرعية والمنتجات
    for (const subCatData of cleaningData.subCategories) {
      console.log(`📁 معالجة القسم الفرعي: ${subCatData.name}`);
      ...
      if (!subCategory) {
        ...
        console.log(`  ✅ تم إنشاء القسم الفرعي: ${subCatData.name}`);
        totalSubCategories++;
      } else {
        console.log(`  ℹ️  القسم الفرعي موجود مسبقاً: ${subCatData.name}`);
      }
      ...
      for (const product of subCatData.products) {
    ...
    console.log(`📊 إجمالي الأقسام الفرعية المضافة: ${totalSubCategories}`);
